...
class PlayerHandler {
    ...
    async getPlayerInfo(guildId) {
        try {
            ...
            if (!thumbnail) {
                console.warn("⚠️ Thumbnail not available, using fallback.");
            }
    ...
    initializeEvents() {
        ...
        this.client.riffy.on('queueEnd', async (player) => {
            try {
                ...
                player.isAutoplay = !!serverConfig?.settings?.autoplay;
                const autoLeave = serverConfig?.settings?.autoLeave ?? true;

                console.log(
                    `🎵 Queue End | Autoplay: ${player.isAutoplay ? 'ON' : 'OFF'} | AutoLeave: ${autoLeave ? 'ON' : 'OFF'}`
                );
                ...
                console.log('🎵 Queue ended - staying in voice channel (AutoLeave OFF).');
                return;
            } catch (error) {
